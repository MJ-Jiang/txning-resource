...
import pytest
from data import find_any_category_with_items
from common_asserts import assert_paging, assert_content_card
...
def test_channels(api):
    ...
    r = api.get(f"/channels/{cid}", params={"limit": 5})
    data = r.json()

    assert_paging(data)

    if data["items"]:
        assert_content_card(data["items"][0])


def test_channels_order(api):
    cid = find_any_category_with_items(api)
    if not cid:
        pytest.skip()

    r = api.get(f"/channels/{cid}", params={"limit": 10})
    items = r.json()["items"]
