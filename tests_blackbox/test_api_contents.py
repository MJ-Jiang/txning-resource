...
#     assert isinstance(data.get("items"), list)
import pytest
from data import find_any_content_id
from common_asserts import assert_paging


def test_contents_requires_category(api):
    r = api.get("/contents")
    assert r.status_code == 422


def test_contents_list(api):
    found = find_any_content_id(api)
    if not found:
        pytest.skip()
    ...
    r = api.get("/contents", params={"category": cid})
    data = r.json()

    assert_paging(data)


def test_content_detail(api):
    found = find_any_content_id(api)
    if not found:
        pytest.skip()
